...
func (v *Verifier) CheckSMTP(domain, username string) (*SMTP, error) {
	...
	ret.CatchAll = true
	// Checks the delivery ability of a randomly generated address in order
	// to verify the existence of a catch-all, etc.
...
// Generates a new available SMTP client
...
// Timeout wrapper for smtp.Dial.
// It attempts to dial an SMTP server (socks5 proxy supported) and
// fails with a timeout if timeout is reached while attempting to establish a new connection
...
// Generates a random email address using the domain passed
// Used primarily for checking the existence of a catch-all address
...
// Connects to the address on the named network address
func establishConnection(addr string) (net.Conn, error) {
	return net.Dial("tcp", addr)
}

// Connects to the address on the named network address via proxy protocol
